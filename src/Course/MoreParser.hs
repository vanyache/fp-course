...
tok ::
  Parser a
  -> Parser a
tok pa = lift2 const pa spaces

tok' :: Parser a -> Parser a
tok' pa = bindParser (\ s -> bindParser (\ _ -> valueParser s ) spaces) pa

tok'' :: Parser a -> Parser a
tok'' pa = (\ s ->  (\ _ -> valueParser s ) =<< spaces) =<< pa
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok start end pa = between (charTok start) (charTok end) pa
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma start end pa = betweenCharTok start end (sepby pa (charTok ',') )