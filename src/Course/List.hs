...
filter ::
  ...
  -> List a
  -> List a
filter pred = foldRight (\ el -> if pred el then (el:.) else id ) Nil
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 = lengthGT4Inc 1


lengthGT4Inc :: Int -> List a -> Bool
lengthGT4Inc _ Nil = False
lengthGT4Inc count (_:.xs) = if count > 4 
                             then True 
                             else lengthGT4Inc (count + 1) xs
-- | Reverse a list.
--
-- >>> reverse Nil
-- []
