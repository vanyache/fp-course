...
instance Applicative List where
  ...
  (<*>) ::
    ...
    -> List a
    -> List b
  (<*>) fl l = foldRight (\ f acc -> (f <$> l) ++ acc) Nil fl
...
sequence ::
  ...
  List (f a)
  -> f (List a)
sequence = foldRight (\fa fla -> (:.) <$> fa <*> fla) (pure Nil)
...
filtering ::
  ...
  -> List a
  -> f (List a)
filtering pred = foldRight (\a fla -> ((++) <$> (\ isTrue -> if isTrue then (a:.Nil) else Nil) <$> (pred a)) <*> fla) (pure Nil)
-----------------------
-- SUPPORT LIBRARIES --
-----------------------
