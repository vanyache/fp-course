...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f (P p)= P (\ input ->   f <$> (p input))
...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f (P p) = P (\ input ->  onResult (p input) (\ input' a -> parse (f a) input'  ))
...
list1 ::
  Parser a
  -> Parser (List a)
list1 pa =  bindParser (\a -> bindParser (\b -> valueParser (a:.b) ) (list pa)) pa
...
firstNameParser ::
  Parser Chars
firstNameParser = bindParser (\ u -> bindParser (\ l -> valueParser (u:.l)) (list lower)) upper
...
surnameParser ::
  Parser Chars
surnameParser = bindParser (\ u -> bindParser (\ m -> bindParser (\l -> valueParser (u:. (m ++ l)) )  (list lower)) (thisMany 5 lower)) upper
...
phoneParser ::
  Parser Chars
phoneParser =   bindParser (\ d -> bindParser(\ pb -> bindParser (\ _ -> valueParser (d:.pb)) (is '#')) phoneBodyParser) digit

phoneParser' :: Parser Chars
phoneParser' = lift3 (\ d pb _ -> d:.pb) digit phoneBodyParser (is '#')
...
personParser ::
  Parser Person
personParser = bindParser (\ a -> 
                  bindParser(\ fn -> 
                      bindParser (\ sn -> 
                          bindParser (\ s->
                              bindParser (\ p -> valueParser (Person a fn sn s p))
                              (spaces1 >>> phoneParser))
...
instance Functor Parser where
  (<$>) ::
    ...
    -> Parser a
    -> Parser b
  (<$>) f pa = bindParser (\ a -> valueParser (f a)) pa
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  (<*>) pab pa = bindParser (\ab -> ab <$> pa) pab
