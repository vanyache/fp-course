...
findLeft p (ListZipper l a r) = 
  case break p l of 
    (_, Nil) -> IsNotZ
    (w', (x:.xs)) -> IsZ (ListZipper xs x (w' ++ (a:.r)))
...
findRight p (ListZipper l a r) = 
  case break p r of 
    (_, Nil) -> IsNotZ
    (w', (x:.xs)) -> IsZ (ListZipper ((reverse w') ++ (a:.l)) x xs)
...
moveLeftLoop (ListZipper Nil a r) = 
  let (x:.xs) = reverse r in
  ListZipper (xs ++ (a:.Nil)) x Nil
...
moveRightLoop (ListZipper Nil a Nil) = ListZipper Nil a Nil
moveRightLoop (ListZipper l a Nil) = 
  let (x:.xs) = reverse l in
  ListZipper Nil x (xs ++ (a:.Nil))
...
moveLeftN n lz = 
  if n > 0
  then moveReallyLeftN n lz
  else moveReallyRightN (-n) lz 
...
moveReallyLeftN ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
moveReallyLeftN n lz = 
  case moveLeft lz of 
    IsZ lz' -> if (n > 1) then moveReallyLeftN (n - 1) lz' else IsZ lz'
    IsNotZ -> IsNotZ
...
moveReallyRightN ::
    ...
    -> ListZipper a
    -> MaybeListZipper a
moveReallyRightN n lz = 
  case moveRight lz of 
    IsZ lz' -> if (n > 1) then moveReallyRightN (n - 1) lz' else IsZ lz'
    IsNotZ -> IsNotZ
...
moveLeftN' n lz =
  ...
  then 
    if n > 0
    then moveReallyLeftN' n n lz
    else Right lz
  else moveReallyRightN' (-n) (-n) lz
...
moveReallyLeftN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveReallyLeftN' nstart n lz=
  case moveLeft lz of 
    IsZ lz' -> if (n > 1) then moveReallyLeftN' nstart (n - 1) lz' else Right $ lz'
    IsNotZ -> Left (nstart - n)
...
moveReallyRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveReallyRightN' nstart n lz=
  case moveRight lz of 
    IsZ lz' -> if (n > 1) then moveReallyRightN' nstart (n - 1) lz' else Right $ lz'
    IsNotZ -> Left (nstart - n)
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n (ListZipper Nil a r) = moveRightN n (ListZipper Nil a r)
nth n lz =
  let lz' = moveToMostLeft lz in
  nth n lz'
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft new (ListZipper l a r) = ListZipper (a:.l) new r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight new (ListZipper l a r) = ListZipper l new (a:.r)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper lf af rf) (ListZipper l a r) = ListZipper (zipWith id lf l)  (af a) (zipWith id rf r)
...
instance Extend ListZipper where
  (<<=) f lz = 
    let lzl = unfoldr (\ mlz -> 
              case mlz of 
                IsNotZ -> Empty
                IsZ lz' -> Full (lz', moveLeft lz')
              ) (moveLeft lz)
    in
    let lzr = unfoldr (\ mlz -> 
              case mlz of 
                IsNotZ -> Empty
                IsZ lz' -> Full (lz', moveRight lz')
              ) (moveRight lz)
    in
    ListZipper (f <$> lzl) (f lz) (f <$> lzr)
...
instance Extend MaybeListZipper where
  (<<=) _ IsNotZ = IsNotZ
  (<<=) f (IsZ lz) = IsZ  ( (convertToLzFunc f) <<= lz) 

convertToLzFunc ::
  (MaybeListZipper a -> b) -> ListZipper a -> b
convertToLzFunc f lz = f (IsZ lz)
...
instance Traversable ListZipper where
  traverse f (ListZipper l a r ) = 
    let flb = sequence (f <$> l) in
    let fb = f a in
    let frb = sequence (f <$> r) in
    
    ListZipper <$> flb <*> fb <*> frb
